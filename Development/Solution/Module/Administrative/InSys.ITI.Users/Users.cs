    ...
    public class Users : BaseModule
    {
        ...
        public override ReturnSet LoadForm()
        {
            ...
            try
            {
                using (var db = new UsersDb(Session))
                {
                    var data = db.Single<vUsers>("select tu.*, tp.Name Employee, tr.Name Roles, tsq.Name SecretQuestion from dbo.tUsers tu" +
                        " left join dbo.tEmployee te on tu.ID_Employee = te.ID" +
                        " left join dbo.tRoles tr on tu.ID_Roles = tr.ID" +
                        " left join dbo.tPersona tp on te.ID_Persona = tp.ID" +
                        " left join dbo.tSecretQuestion tsq on tu.ID_SecretQuestion = tsq.ID where tu.ID = {0} and tu.ID > 1", ID);
                    data.Password = null;
