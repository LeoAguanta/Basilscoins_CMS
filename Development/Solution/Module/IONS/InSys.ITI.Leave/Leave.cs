        ...
        public override ReturnSet LoadList()
        {
            try
            {
                using (var db = new LeaveDb(Session))
                {
                    ...
                    var data = db.QueryTable<vLeave>(@"(SELECT l.Id, l.RefNum, 
                                        l.ID_Company, c.Name Company, 
                                        l.ID_Employee, e.EmployeeName, 
                                        StartDate, EndDate, FiledDate,
                                        l.ID_FilingStatus, fs.Name AS Status, 
                                        l.IsPosted, l.Reasons, l.ModifiedAt,
                                        l.ID_ModifiedBy, e2.EmployeeName AS LastModifiedBy,
                                        l.ID_CreatedBy, e3.EmployeeName AS CreatedBy,
                                        l.ApproverLevel, l.CreatedAt,
                                        l.ID_LeavePayrollItem,pi.Name LeavePayrollitem
                                        FROM tLeave l
                                        INNER JOIN tFilingStatus fs ON fs.ID = l.ID_FilingStatus
                                        INNER JOIN vEmployees e ON e.ID_Employee = l.ID_Employee
                                        INNER JOIN vEmployees e2 ON e2.ID_Employee = l.ID_ModifiedBy
                                        INNER JOIN vEmployees e3 ON e3.ID_Employee = l.ID_CreatedBy
                                        LEFT JOIN dbo.tCompany c ON l.ID_Company = c.ID
                                        LEFT JOIN dbo.tPayrollItem pi ON l.ID_LeavePayrollItem = pi.ID
                                        WHERE l.ID_Employee = {0})a", to, Session.ID_Employee);
        ...
        public override ReturnSet LoadForm()
        {
            try
            {
                using (var db = new LeaveDb(Session))
                {
                    ...
                    var data = db.Single<vLeave>(@"SELECT l.Id, l.RefNum, 
                                        l.ID_Company, c.Name Company, 
                                        l.ID_Employee, e.EmployeeName, 
                                        StartDate, EndDate, FiledDate,
                                        l.ID_FilingStatus, fs.Name AS Status, 
                                        ...
                                        INNER JOIN vEmployees e3 ON e3.ID_Employee = l.ID_CreatedBy
                                        LEFT JOIN dbo.tCompany c ON l.ID_Company = c.ID
                                        LEFT JOIN dbo.tPayrollItem pi ON l.ID_LeavePayrollItem = pi.ID where l.id = {0}", paramId); // where l.id = {0}

                    //payrollitem
                    var payrollItems = base.LoadLookup<tPayrollItem>(db, "SELECT * FROM tPayrollItem WHERE IsActive = 1 and ID in (26,27)");
