angular.module('app')
    .controller('SupplementarySettingsCivilStatus', ['$scope', '$controller', '$priv', '$state', function ($s, $c, $priv, $st) {
        $c('insysTableController', { $scope: $s });
        $s.MenuPrivileges = $priv.Data;
        $s.Schema = {};
        $s.TableSchema = [];
        $s.RecordID = $st.params.ID == undefined ? '' : $st.params.ID;
        $s.MenuCode = 'SupplementarySettingsCivilStatus';
        ...
        $s.Init = function () {
            if ($s.RecordID != '') {
                ...
                $s.LoadTable($s.tblOptions, 'LoadList', 'CivilStatus', { MenuCode: $s.MenuCode }).then(function (ret) {
                    if (ret.Type == 2) {
                        $s.SetSystemStatus(ret.Message, 'error');
                    } else {
                        $s.SetSystemStatus('Ready');
        ...
        $s.LoadForm = function () {
            return $s.Request('LoadForm', { ID: $s.RecordID, MenuCode: $s.MenuCode }, 'CivilStatus');
        }

        $s.saveForm = function () {
            if ($s.IsTabsValid('form.civilstatus')) {
                ...
                $s.Request('SaveForm', { Data: $s.Schema, MenuCode: $s.MenuCode }, 'CivilStatus').then(function (ret) {
                    if (ret.Type == 2) {
                        $s.SetSystemStatus(ret.Message, 'error');
                        $s.$apply();
                    } else {
...
$st.go($st.current.name, { ID: ret.Data }, { reload: true });
        ...
        $s.delete = function () {
            $s.deleteRow('DeleteRecord', 'CivilStatus', { MenuCode: $s.MenuCode });
        }
