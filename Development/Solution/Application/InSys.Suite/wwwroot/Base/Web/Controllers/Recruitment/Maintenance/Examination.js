angular.module('app')
    .controller('RecruitmentMaintenanceExamination', ['$scope', '$controller', '$priv', '$state', function ($s, $c, $priv, $st) {
        $c('insysTableController', { $scope: $s });
        $s.MenuPrivileges = $priv.Data;
        $s.RecordID = $st.params.ID == undefined ? '' : $st.params.ID;
        $s.myController = 'Examination';
        $s.TableSchema = [];

        $s.MenuCode = 'RecruitmentMaintenanceExamination';
        ...
        $s.saveForm = function () {
            if ($s.IsTabsValid('form.examination')) {
