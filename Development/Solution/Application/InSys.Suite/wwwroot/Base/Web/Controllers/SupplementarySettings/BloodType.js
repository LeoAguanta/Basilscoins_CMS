angular.module('app')
    .controller('SupplementarySettingsBloodType', ['$scope', '$controller', '$priv', '$state', function ($s, $c, $priv, $st) {
        $c('insysTableController', { $scope: $s });
        $s.MenuPrivileges = $priv.Data;
        $s.Schema = {};
        $s.TableSchema = [];
        $s.RecordID = $st.params.ID == undefined ? '' : $st.params.ID;
        $s.MenuCode = 'SupplementarySettingsBloodType';
        ...
        $s.Init = function () {
            if ($s.RecordID != '') {
                ...
                $s.LoadTable($s.tblOptions, 'LoadList', 'BloodType', { MenuCode: $s.MenuCode }).then(function (ret) {
                    if (ret.Type == 2) {
                        $s.SetSystemStatus(ret.Message, 'error');
                    } else {
                        $s.SetSystemStatus('Ready');
        ...
        $s.LoadForm = function () {
            return $s.Request('LoadForm', { ID: $s.RecordID, MenuCode: $s.MenuCode }, 'BloodType');
        }

        $s.saveForm = function () {
            if ($s.IsTabsValid('form.bloodtype')) {
                ...
                $s.Request('SaveForm', { Data: $s.Schema, MenuCode: $s.MenuCode }, 'BloodType').then(function (ret) {
                    if (ret.Type == 2) {
                        $s.SetSystemStatus(ret.Message, 'error');
                        $s.$apply();
                    } else {
...
$st.go($st.current.name, { ID: ret.Data }, { reload: true });
        ...
        $s.delete = function () {
            $s.deleteRow('DeleteRecord', 'BloodType', { MenuCode: $s.MenuCode })
        }
