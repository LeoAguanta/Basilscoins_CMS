angular.module('app')
    .controller('RecruitmentMaintenanceRecruitmentStatus', ['$scope', '$controller', '$priv', '$state', function ($s, $c, $priv, $st) {
        $c('insysTableController', { $scope: $s });
        $s.MenuPrivileges = $priv.Data;
        $s.RecordID = $st.params.ID == undefined ? '' : $st.params.ID;
        $s.myController = 'RecruitmentStatus';
        $s.Schema = {};
        $s.dummy = {};
        $s.RecruitmentStatusTypes = {};
        $s.TableSchema = [];
        $s.MenuCode = 'RecruitmentMaintenanceRecruitmentStatus';
        $s.tblOptions = {
            Columns: [
                { Name: 'ID', Label: '#' },
            ...
            Filters: [
                ...
                { Name: 'Type', Type: 1, ControlType: 'select', Label: 'Type', Options: { controller: $s.myController, method: 'LoadLookUp', parameter: { Name: 'RecruitmentStatusTypes', MenuCode: $s.MenuCode } } },
        ...
        $s.GetLookUp = function () {
            $s.Request('LoadLookUp', { Name: 'RecruitmentStatusTypes', MenuCode: $s.MenuCode }, $s.myController).then(function (ret) {
                if (ret.Type == 2) {
                    $s.SetSystemStatus(ret.Message, 'error');
                } else {
                    $s.RecruitmentStatusTypes = ret.Data.Rows;
        ...
        $s.saveForm = function () {
            $s.Schema.Description = $s.Schema.Type == 1 ? 'Application Status' : 'MRF Status';

            if ($s.IsTabsValid('form.recruitmentstatus')) {
