...
angular.module('app')
    .controller('BaseController', ['$scope', 'DataService', '$sce', 'Session', '$uibModal', '$rootScope', '$timeout', '$state', function ($s, $r, $sce, $ss, $uibModal, $rs, $timeout, $state) {
        ...
        $s.SetSystemStatus = function (message, type) {
        ...
        $s.SetMaxLengthForVarchar = function (TableSchema) { //POWERED BY BRIAN BUEN ANTONIO REMEMBER ME
            ...
            if (TableSchema != undefined) {
                ...
                console.log(_TableSchema.length);
                var x = 0;
                var input_name = "";
        ...
        $s.SetMaxLengthRequired = function (TableSchema) {
        ...
        $s.IsTabsValid = function (formName, tableSchema, tabId) {
            ...
            if (form.length > 0) {
                ...
                if (tabs.length == 0) {
                    ...
                    form.find('input,textarea,select:visible').not('.listing-footer select, input[type="checkbox"], .insys-filter input,.insys-filter textarea,.insys-filter select,[text-angular] textarea').each(function (idx, elem) {
                        ...
                        var column = Enumerable.From(tableSchema).Where(function (x) { return x.ColumnName.toLowerCase() == elemName.toLowerCase() }).FirstOrDefault();
                        if (column != undefined && column != null) {
                            if (!column.AllowNull)
                                if ($(elem).val() == "" || $(elem).val() == null || $(elem).val() == undefined || $(elem).val() == 'object:null') {
                    ...
                    if (errorCol.length > 0) {
                        ...
                        for (var idx in errorCol) {
                            $(elem).filter('[name=' + errorCol[idx] + ']').each(function (xx, elReq) {
                                if ($(elReq).val() == "" || $(elReq).val() == null || $(elReq).val() == undefined || $(elReq).val() == 'object:null') {
                        ...
                        $s.SetSystemStatus(errorCol.join(', ') + ' is required.', 'warning');
                    }
                } else {
    ...
    .controller('insysTableController', ['$scope', '$controller', '$state', '$timeout', '$q', function ($s, $c, $st, $t, $q) {
        ...
        $s.RefreshLoadTable = function () {
            $s.LoadTable($s.current.options, $s.current.action, $s.current.controller, $s.current.xParams);
        }
        ...
        $s.getSelectedRows = function (tblOptions) {
            if (tblOptions == undefined || tblOptions == null) {
                return Enumerable.From($s.tblOptions.Data.Rows).Where(function (x) { return x.IsChecked == true }).Select(function (x) { return x.ID }).ToArray();
            }
            else {
                return Enumerable.From(tblOptions.Data.Rows).Where(function (x) { return x.IsChecked == true }).Select(function (x) { return x.ID }).ToArray();
            }
        }
        ...
        $s.sortData = function (colName, label) {
            ...
            $s.RefreshLoadTable().then(function () {
                $s.SetSystemStatus('Ready');
            });
        }
    ...
    .controller('insysDatePickerController', ['$scope', '$element', '$attrs', '$compile', function ($s, $e, $a, $compile) {
        ...
        $s.generateTemplate = function () {
            return '<div id="insysDatePicker"><div class="idp-header"><div class="idp-select"> <select name="insysDatePicker-month" ng-model="data.currentMonth" id="insysDatePicker-month" ng-change="jump()" ng-options="item.ID as item.Name for item in months"></select> <select name="insysDatePicker-year" ng-model="data.currentYear" id="insysDatePicker-year" ng-change="jump()" ng-options="item for item in years"></select></div><div class="idp-btn"> <i id="insysDatePicker-previous" class="fa fa-chevron-left" ng-click="previous()" title="Previous"></i> <i id="insysDatePicker-next" class="fa fa-chevron-right" ng-click="next()" title="Next"></i> <i id="insysDatePicker-today" class="fa fa-calendar" ng-click="goToday()" title="Current Day"></i></div></div><div class="idp-body"><table><thead><tr><th>Sun</th><th>Mon</th><th>Tue</th><th>Wed</th><th>Thu</th><th>Fri</th><th>Sat</th></tr></thead><tbody id="insysDatePicker-body"><tr><td ng-repeat="n in [].constructor(firstDay) track by $index"></td><td ng-class="{\'bg-info\': IsDateActive($index + 1), \'disabled\': IsDisabled($index + 1)}" ng-mouseup="getCalendarValue($index + 1)" ng-repeat="n in [].constructor(7 - firstDay) track by $index" ng-bind="$index + 1"></td></tr><tr><td ng-class="{\'bg-info\': IsDateActive($index + (8-firstDay)), \'disabled\': IsDisabled($index + (8-firstDay))}" ng-mouseup="getCalendarValue($index + (8-firstDay))" ng-repeat="n in [].constructor(7) track by $index" ng-bind="$index + (8-firstDay)"></td></tr><tr><td ng-class="{\'bg-info\': IsDateActive($index + (8-firstDay) + 7), \'disabled\': IsDisabled($index + (8-firstDay) + 7)}" ng-mouseup="getCalendarValue($index + (8-firstDay) + 7)" ng-repeat="n in [].constructor(7) track by $index" ng-bind="$index + (8-firstDay) + 7"></td></tr><tr><td ng-class="{\'bg-info\': IsDateActive($index + (8-firstDay) + 14), \'disabled\': IsDisabled($index + (8-firstDay) + 14)}" ng-mouseup="getCalendarValue($index + (8-firstDay) + 14)" ng-repeat="n in [].constructor(7) track by $index" ng-bind="$index + (8-firstDay) + 14"></td></tr><tr><td ng-class="{\'bg-info\': IsDateActive($index + (8-firstDay) + 21), \'disabled\': IsDisabled($index + (8-firstDay) + 21)}" ng-if="($index + (8-firstDay) + 20) < daysInMonth(data.currentMonth, data.currentYear)" ng-mouseup="getCalendarValue($index + (8-firstDay) + 21)" ng-repeat="n in [].constructor(7) track by $index" ng-bind="$index + (8-firstDay) + 21"></td></tr><tr><td ng-class="{\'bg-info\': IsDateActive($index + (8-firstDay) + 28), \'disabled\': IsDisabled($index + (8-firstDay) + 28)}" ng-if="($index + (8-firstDay) + 27) < daysInMonth(data.currentMonth, data.currentYear)" ng-mouseup="getCalendarValue($index + (8-firstDay) + 28)" ng-repeat="n in [].constructor(7) track by $index" ng-bind="$index + (8-firstDay) + 28"></td></tr></tbody></table></div></div>';
        }
        $s.daysInMonth = function (iMonth, iYear) {
            return 32 - new Date(iYear, iMonth, 32).getDate();
        }
        $s.generateYears(1990, $s.today.getFullYear() + 11);
            ...
            link: function ($s, $e, $attr) {
                ...
                $s.openLookup = function (ev) {
                    ...
                    if ($('#lookup-handle').length == 0) {
                        ...
                        $('body').append($compile('<div id="lookup-handle" class="lookup-table"><div id="lookup-handleheader"><div class="lookup-title">' + $s.lookupData.label + '</div><i class="fas fa-ellipsis-v"></div><div ng-include="\'' + tmpPath + '/Web/Directives/insysTable.tmpl.html\'" include-replace></div></div>')($s));
        ...
        $s.Submit = function () {
            $s.Request('SaveHeirarchy', { Data: $s.Org || {}, MenuCode: 'PeopleOrganizationCompany' }, 'Company').then(function (ret) {
                if (ret.Type == 2) {
                    ...
                    $s.Prompt('Save successfull.', 'Success');
